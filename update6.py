...
def create_pdf(summary_df):
    ...
    for col in partner_summary.columns: pdf.cell(35, 8, clean_for_pdf(str(col)), 1)
    pdf.ln()
    for partner, row in partner_summary.iterrows():
        pdf.cell(60, 8, clean_for_pdf(str(partner)), 1)
        for val in row: pdf.cell(35, 8, clean_for_pdf(str(val)), 1)
        pdf.ln()
    pdf.ln(10)
    pdf.set_font("Arial", "B", 12)
    ...
    for i, col in enumerate(cols): pdf.cell(widths[i], 8, clean_for_pdf(col), 1)
    pdf.ln()
    for _, row in summary_df.iterrows():
        for i, col in enumerate(cols): pdf.cell(widths[i], 8, clean_for_pdf(str(row[col])), 1)
        pdf.ln()
    ...
    if search_name:
        ...
        if not node.empty:
            ...
            if s_dist > 800:
                ...
                if is_dist_ok and is_port_ok: stat, reas = "Can Deploy", "OK"
                elif not is_dist_ok: stat, reas = "NOK", f"Over meter {int(d) if d else 'Unknown'} m"
                else: stat, reas = "NOK", f"Full Port ({int(n['act'])})"
        ...
        else: res["connected_reason"] = "Node Not Found"
    if res["connected_status"] != "Can Deploy":
        nodes_df["temp_dist"] = nodes_df.apply(lambda r: haversine(cust_lat, cust_lon, r["Latitude"], r["Longitude"]), axis=1)
        candidates = nodes_df[nodes_df["temp_dist"] < 600].sort_values("temp_dist").head(TOP_FALLBACK_NODES)
        for _, n in candidates.iterrows():
            ...
            if d and d <= MAX_DISTANCE and n["act"] < MAX_CAPACITY:
                res["recommended_map_obj"] = {"name": n["node_name"], "lat": n["Latitude"], "lon": n["Longitude"], "dist": d, "geom": g, "status": "Can Deploy", "reason": "OK"}
                break
    return res
...
def draw_map(res, conn=None, reco=None):
    ...
    folium.Marker([res["cust_lat"], res["cust_lon"]], tooltip=f"{res['customer_name']}", icon=folium.Icon(color="blue")).add_to(m)
    for obj, color in [(conn, "red"), (reco, "green")]:
        if obj and obj.get("lat"):
            ...
            if obj.get("geom"): folium.GeoJson(obj["geom"], style_function=lambda x, c=color: {"color": c, "weight": 5}).add_to(m)
    return m
...
with t1:
    ...
    if col_b1.button("Run Batch", type="primary"):
        try:
            ...
            cust_data = pd.read_excel("customers.xlsx") 
            ...
            nodes["node_name_upper"] = nodes["node_name"].str.strip().str.upper()
            ...
            for i, (_, r) in enumerate(new_custs.iterrows()):
                ...
                results.append(res)
                summary_rows.append({"Customer Name": res["customer_name"], "Partner": res["partner"], "Lat": res["cust_lat"], "Long": res["cust_lon"], "Status": res["connected_status"], "Reason": res["connected_reason"], "Connected Node": res["connected_name"], "Distance (m)": res["connected_dist"], "Recommended": res["recommended_map_obj"]["name"] if res["recommended_map_obj"] else "-"})
    ...
    if st.session_state.batch_done:
        ...
        for c in ["Partner", "Status", "Reason"]: df_full[c] = df_full[c].astype(str).fillna("-")
        ...
        with pd.ExcelWriter(xlsx_io, engine='xlsxwriter') as writer:
            ...
            for i, col in enumerate(f_df.columns): writer.sheets['Report'].set_column(i, i, 20)
        c1.download_button("Download Excel Report", data=xlsx_io.getvalue(), file_name="Report.xlsx")
        try:
            pdf_data = create_pdf(f_df)
            c2.download_button("Download PDF Report", data=pdf_data, file_name="Technical_Report.pdf", mime="application/pdf")
...
with t2:
    ...
    if sc1.button("Run Single", type="primary"):
        if s_la and s_lo:
            # Changed from csv to excel
            cust_db = pd.read_excel("customers.xlsx")
            n_s = pd.read_csv("nodes.csv").merge(cust_db.groupby("node_name").size().reset_index(name="act"), on="node_name", how="left").fillna(0)
            n_s["node_name_upper"] = n_s["node_name"].str.strip().str.upper()
